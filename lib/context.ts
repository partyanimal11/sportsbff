...
export function retrieveContextChunks(query: string, lensId: string, limit = 12, dramaMode = false): string[] {
  ...
  // Character-comparison reminder for player questions
  ...
  if (lensShow) {
    const chars = lensShow.characters?.slice(0, 10).map((c) => `${c.name} (${c.archetype ?? ''})`).join('; ') ?? '';
...
export function buildSystemPrompt({ lensId, userMessage, dramaMode = false }: BuildPromptInput): string {
  ...
  const chunks = retrieveContextChunks(userMessage, lensId, dramaMode ? 16 : 12, dramaMode);
